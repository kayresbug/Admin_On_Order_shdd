...
//        }
    ...
    public void print(PrintOrderModel printOrderModel) {
        ...
        if (printOrderModel.getType().equals("service")){
            str_table = str_table + "번 호출";
        }else {
            if (printOrderModel.getOrderType().equals("cash")) {
                str_table = str_table + "번 현금";
            } else {
                str_table = str_table + "번 주문";
            }
        }
...
// body
            ...
            if (printOrderModel.getType().equals("service")){
                ...
                for (int i = 0; i < order.size(); i++) {
                    builder2.addTextSize(2, 2);
                    if (Integer.parseInt(order.get(i).getMenuNo()) < 6021 || Integer.parseInt(order.get(i).getMenuNo()) == 6030 || Integer.parseInt(order.get(i).getMenuNo()) == 6031
                            || Integer.parseInt(order.get(i).getMenuNo()) == 6033  || Integer.parseInt(order.get(i).getMenuNo()) == 6034 || Integer.parseInt(order.get(i).getMenuNo()) == 6036
                            || Integer.parseInt(order.get(i).getMenuNo()) == 6055) {
//                    if (Integer.parseInt(order.get(i).getMenuNo()) < 6014 || Integer.parseInt(order.get(i).getMenuNo()) > 6029 || Integer.parseInt(order.get(i).getMenuNo()) < 6039) {
                        if (!order.get(i).getName().equals("공기밥")) {
                            builder2.addText(order.get(i).getName());

                        i_count2 = i_count2 + 1;

                    builder2.addFeedLine(1);
                    builder2.addText("                 " + order.get(i).getCount() + "개");
                    }
                    builder2.addFeedLine(1);
                    builder2.addTextSize(1, 1);
                    builder2.addText("------------------------------------------");
...
// body
            ...
            if (printOrderModel.getType().equals("service")){
                builder3.addTextSize(2, 2);
                builder3.addText(order.get(0).getName());
                builder3.addFeedLine(1);
                ...
                for (int i = 0; i < order.size(); i++) {
                    builder3.addTextSize(2, 2);
//                builder.addTextAlign(Sam4sBuilder.ALIGN_LEFT);
                    if ((6020 < Integer.parseInt(order.get(i).getMenuNo())  && Integer.parseInt(order.get(i).getMenuNo()) < 6042)) {
                        if (!order.get(i).getName().equals("공기밥")) {
                            builder3.addText(order.get(i).getName());
                            i_count3 = i_count3 + 1;
                            builder3.addFeedLine(1);
                            builder3.addText("                 " + order.get(i).getCount() + "개");
                        }
                        builder3.addFeedLine(1);
                        builder3.addTextSize(1, 1);
                        builder3.addText("------------------------------------------");
...
// footer
    ...
    public void rePrint(String orderTime, String authdate, String authnum, String orderbody, String table, String price, String vanTr, String cardbin){
        ...
        try {
            ...
            builder.addText(prevAuthDate);
            builder.addFeedLine(1);
            builder.addText("신화돌담집");
            builder.addFeedLine(1);
            builder.addText("김성보 \t");
            builder.addText("273-22-01104 \t");
            builder.addText("Tel : 064-909-2205");
            builder.addFeedLine(1);
            builder.addText("제주특별자치도 서귀포시 인덕면 신화역사로304번길 98");
            builder.addFeedLine(1);

            builder.addText("------------------------------------------");
            builder.addFeedLine(1);
            builder.addText("TID:\t");
            builder.addText("AT0298910A \t");
            builder.addText("A-0000 \t");
            builder.addText("0017");
            builder.addFeedLine(1);
//            builder.addText("카드종류: ");
